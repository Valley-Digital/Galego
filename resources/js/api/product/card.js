...
function insertItemInList(productId, element) {
    let keep = true,
        ...
        quantity = $(element).closest("div[id^='Product_']").find('#qtdProdutoCart_'+productId).val() || 1;
    ...
    if (keep) {
        callAjaxInsertItemInList(productId, variationSelected, quantity, element);
    }
    else {
...
export function insertItemInCart(productId, element, showSideBar, signature = false) {
    let $parent = $(element).closest("div[id^='Product_']"),
        ...
        quantity = $parent.find('#qtdProdutoCart_'+productId).val() || 1;

    if (variationSelected.status) {
        callAjaxInsertItemInCart(productId, variationSelected.ids, quantity, element, showSideBar);
    }
    else {
        _alert("", "Variação não selecionada!", "warning");
        $(element).removeClass('loading');
